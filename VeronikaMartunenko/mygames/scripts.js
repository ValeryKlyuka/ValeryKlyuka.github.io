...
function beginRandNumb() {
  ...
  rendAnswer.style.visibility = "visible";
  randStart.value = "начать заново";
}
function ancwerRandNumb() {
  tryes++
  ...
  randOut.value ="Мое число больше. Попыток:" + tryes;
    ...
    rendAnswer.style.visibility = "hidden";
  }
}else {
  alert("Не получилось  :(")
}
...
function tap(box){
  if(document.getElementById(box.id).value == '' && tictacstart == true){
    ...
    else if (field[2] == 'X' && field[6] == 'X' && field[8] == 'X'){tictacprogress.value = 'X победил!';tictacstart = false;}
    ...
    else if (field[2] == 'O' && field[6] == 'O' && field[8] == 'O'){tictacprogress.value = 'O победил!';tictacstart = false;}
